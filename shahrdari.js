...
const inputs = [customCeilingInput, sitePercentInput, outsideInput, herasiInput];
...
[customCeilingInput, outsideInput, herasiInput].forEach(formatNumberInput);
...
function selectGeneral() {
    ...
    if (ceilingMode === "fixed") { selectedLimit = 15000000; document.getElementById("fixedCeilingText").textContent = "15,000,000 ریال"; }
}
function selectSpecialist() {
    ...
    if (ceilingMode === "fixed") { selectedLimit = 40000000; document.getElementById("fixedCeilingText").textContent = "40,000,000 ریال"; }
}
...
function calculate() {
    ...
    const technicalValue = 727000;
    const base = outside * ((100 - percent) / 100);
    calculatedAmount = Math.floor(outside * 0.7 + technicalValue + herasi);
    selectedLimit = ceilingMode === "custom" ? getNumber(customCeilingInput.value) : (specialty === "specialist" ? 40000000 : 15000000);
...
function getFields() {
    ...
    if (customTechnicalInput && !customTechnicalInput.disabled) fields.push(customTechnicalInput);
